...
# Import the other python scripts and settings.json
...
from AntSDR_receiver import AntSDRReceiver
...
def receiving_thread_function(web_intface):
    """
    Function used to tell receiver to get each sample package, runs on its own thread
    """
    running = False
    print('RECEIVER THREAD FUNCTION STARTED')
    while True:
        if running:
            web_intface.module_receiver.next_iq_frame()
            if web_intface.module_signal_processor.run_processing == False:
                break
        else:
            if web_intface.module_signal_processor.run_processing == True:
                running = True
                print('STARTING UP iq collection')
            time.sleep(0.1)
            if VERBOSE:
                print('No running command yet for receiver thread')


#Set sampling rate as a global variable
sampling_rate = dsp_settings.get("sampling_freq", 8e6)


class webInterface():
    """
    Main class containing the web app everything runs from
    """
    def __init__(self):
        ...
        #############################################
        #  Initialize and Configure ant modules  #
        #  Setting parameters #
        #############################################
        ...
        # Instantiate and configure Receiver with configured settings
        self.module_receiver = AntSDRReceiver(samp_rate=sampling_rate, center_freq=490e6, gain_0=32.5, gain_1=44.5,packet_size=1000,n_packets=2016)
        self.module_receiver.packet_size  = dsp_settings.get("n_samps", 1000)
        self.module_receiver.n_packets   = dsp_settings.get("per_package", 2016)
        self.module_receiver.daq_center_freq   = dsp_settings.get("center_freq", 490.0) * 10**6
        self.module_receiver.daq_rx_gain       = [float(dsp_settings.get("gain_0", 32.5)), float(dsp_settings.get("gain_1", 44.5))]
        self.module_receiver.rec_ip_addr       = dsp_settings.get("default_ip", "0.0.0.0")

        # Set up signal processor with correct settings
        ...
        if hasattr(self.module_receiver, 'name'):
            if self.module_receiver.name == 'AntSDR':
                self.module_receiver.start_receiving()
        ...
        self.dsp_timer = None
        self.update_time = 9999     #vad händer om man ändrar denna?
    ...
    def save_configuration(self):
        """
        NOT IN USE FOR ANTSDR, LEFTOVER FROM KRAKEN
        """
        data = {}
    ...
    def start_processing(self):
        """
            Starts data processing, triggered by button in config tab
        ...
        if hasattr(self.module_receiver, 'name'):
            if self.module_receiver.name == 'AntSDR':
                self.module_receiver.next_iq_frame()
                print('ATTEMPTING TO START RECEIVER THREAD FUNCTION')
                self.rec_thread = Thread(target=receiving_thread_function,args=(self,))
    ...
    def stop_processing(self):
        """
        Used to stop receiving and processing of SDR
        """
        self.module_signal_processor.run_processing=False
        if hasattr(self.module_receiver, 'name'):
            if self.module_receiver.name == 'AntSDR':
                self.module_receiver.stop_receiving()
    ...
    def config_daq_rf(self, f0, gain):
        ...
        self.module_receiver.set_if_gain(gain) #gain är vad man ställer in de två antennerna på (inte vad de får in)
...
def read_config_file_dict(config_fname=daq_config_filename):
    """
    Mostly leftover from Kraken, not in use
    """
...
#       Most everything here is to set up the GUI      #
#############################################
...
app.layout = html.Div([
    ...
    html.Div([html.A("Configuration", className="header_active"   , id="header_config"  ,href="/config"),
            html.A("Obsolete tab"       , className="header_inactive" , id="header_spectrum",href="/spectrum"),   
            html.A("Passive Radar" , className="header_inactive" , id="header_doa"     ,href="/pr"),
            html.A("Passive Radar + ADS-B" , className="header_inactive" , id="header_doa_adsb"     ,href="/prnew"),
            ], className="header"),
...
def generate_config_page_layout(webInterface_inst):
    ...
    html.Div([
        html.Div([html.Div([html.Button('Start Processing', id='btn-start_proc', className="btn_start", n_clicks=0)], className="ctr_toolbar_item"),
              ...
              html.Div([html.Button('Obsolete button', id='btn-save_cfg', className="btn_save_cfg", n_clicks=0)], className="ctr_toolbar_item")
    ...
    [
        ...
        html.Div([
                html.Div("Receiver gain (not working)", className="field-label"), 
        ...
        html.Div([
            html.Button('Obsolete button', id='btn-update_rx_param', className="btn"),
        ], className="field"),
        ...
        html.Div([html.Div("Basic Custom DAQ Configuration", id="label_en_basic_daq_cfg"     , className="field-label")]),
            html.Div([
                html.Div("Obsolete something", className="field-label"),                                         
        ...
        html.Div([html.Div("Obsolete something", id="label_en_advanced_daq_cfg"     , className="field-label"),
                dcc.Checklist(options=option     , id="en_advanced_daq_cfg"     ,  className="field-body", value=en_advanced_daq_cfg),
        ], className="field"),        
    ]
    
    print(" ")
    print(webInterface_inst.module_receiver.daq_rx_gain)
    print(" ")
    ...
    daq_subsystem_reconfiguration_options = [ \
        ...
        # Reconfigure Button
        html.Div([
            html.Button('Obsolete button', id='btn_reconfig_daq_chain', className="btn"),
        ], className="field"),
    ]
    ...
    html.Div([
        ...
        html.Div([html.Div("obsolete something:"   , id="label_daq_cpi"           , className="field-label"), html.Div("- ms"        , id="body_daq_cpi"           , className="field-body")], className="field"),
    ...
    dsp_config_card = \
    html.Div([
        html.H2("Passive Radar Configuration (Don't change anything here, use settings.json)", id="init_title_d"),
        ...
        html.Div([html.Div("Max Bistatic Range:"             , id="label_max_bistatic_range"  , className="field-label"), 
        ...
        html.Div([html.Div("Max Doppler shift"             , id="label_max_doppler"  ,style={"display":"inline-block"}, className="field-label"), 
...
###################
# THE SPECTRUM HERE BELOW IS NOT IN USE
###############
...
def generate_adsb_page_layout(webInterface_inst):
    adsb_page_layout = html.Div([
        html.Div([
        dcc.Graph(
...
def func(client, connect):
    ...
    if connect and len(app.clients)==1:
        if VERBOSE:
            print('FETHCING DATA AS INTENDED')
        fetch_dsp_data()
...
def fetch_dsp_data():
    """
    This function is used to fetch each Matrix processed from a sample package, along with information tags
    """


    #print('TRYING TO FETCH DSP DATA')
    if webInterface_inst.started_processing == False:
        if VERBOSE:
            print('DID NOT START PROCESSING YET')
        webInterface_inst.dsp_timer = Timer(1, fetch_dsp_data)
        ...
        #############################################
        #      Fetch new data from back-end ques    #       #TODO: add fetch to ADS-B data here? Or build something similar
        ##############################################################
        try:
        ...
        try:
            ...
            for data_entry in que_data_packet:
                if data_entry[0] == "iq_header":
                    ...
                    if hasattr(webInterface_inst.module_receiver, 'name'):
                        if webInterface_inst.module_receiver.name == 'AntSDR':
        ...
        if (webInterface_inst.pathname == "/config" or webInterface_inst.pathname == "/") and daq_status_update_flag:
            ...
            if VERBOSE:
                print('ATTEMPTING TO PLOT PR')
            plot_pr()
        elif (webInterface_inst.pathname == "/prnew" and doa_update_flag): #or (webInterface_inst.pathname == "/doa" and webInterface_inst.reset_doa_graph_flag):
            plot_adsb()

        webInterface_inst.dsp_timer = Timer(.05, fetch_dsp_data)
        webInterface_inst.dsp_timer.start()
...
@app.callback([Output("page-content"   , "children"),
              ...
              Output("header_doa_adsb"   ,"className")],
              [Input("url"            , "pathname")])
def display_page(pathname):    
    ...
    if pathname == "/":
        ...
        return [generate_config_page_layout(webInterface_inst), "header_active", "header_inactive", "header_inactive", "header_inactive"]
        ...
        #time.sleep(1)
        return [spectrum_page_layout, "header_inactive", "header_active", "header_inactive","header_inactive"]
    elif pathname == "/pr":
        webInterface_inst.module_signal_processor.en_spectrum = False
        plot_pr()
        return [generate_pr_page_layout(webInterface_inst), "header_inactive", "header_inactive", "header_active", "header_inactive"]
    elif pathname == "/prnew":
        webInterface_inst.module_signal_processor.en_spectrum = False
        plot_adsb()
        return [generate_adsb_page_layout(webInterface_inst), "header_inactive", "header_inactive", "header_inactive","header_active"]
...
def plot_spectrum():
    """
    NOT IN USE, FROM KRAKEN LEFTOVER
    """
...
adsb_fig = go.Figure(layout=fig_layout)

X = []
Y = []
x_range = [0,dsp_settings.get("max_bistatic_range", 128)]
Y.append(0)
flight_name = 'N/A'
...
def bistatic_converter(lat,lon):
    ...
    dTxRx = geopy.distance.distance(Tx, Rx).km #For testing: should be 23.something kilometres


    #Calculating bistatic distance
    target = (lat,lon)    #coordinates from ADS-B
    ...
    bistatic_dist = dRxT+dTxT-dTxRx

    return (bistatic_dist)


def read_aircraft():
    """
    Read aircraft from ADSB
    """

    aircrafts = []
    json_file_path = '/run/readsb/aircraft.json'
    with open(json_file_path, 'r') as j:
        json_data = json.loads(j.read())

    if json_data["aircraft"]:
        for i in range(0,len(json_data["aircraft"])):
            if "flight" in json_data["aircraft"][i] and "lat" in json_data["aircraft"][i]:
                lat = json_data["aircraft"][i]["lat"]
                lon = json_data["aircraft"][i]["lon"]
                flight = json_data["aircraft"][i]["flight"]
                bistatic_distance = bistatic_converter(lat,lon)

                ###ALTERED 
                r_block = 2.98e8 / sampling_rate
                bistatic_distance=1000*bistatic_distance/r_block



                aircraft = [bistatic_distance,flight]
                aircrafts.append(aircraft)
            else:
                aircrafts.append([0,"N/A"])
...
aircrafts = read_aircraft()
len_air = len(aircrafts)    #the amount of aircraft ADS-B can find

for i in range(0, len(aircrafts)):
    adsb_fig.add_trace(go.Scatter(x=[aircrafts[i][0]],
                            y=Y,
                            name = aircrafts[len_air-1][1],
                            showlegend=False,
                            visible = True,
                            mode = 'markers',
                            marker = dict(
...
adsb_fig.update_xaxes(title_text="Bistatic distance", range=x_range)
adsb_fig.update_layout(yaxis = dict(title = '', tickvals = []))
...
def plot_adsb():
    ...
    if VERBOSE:
        print("""
    THE DETECTED AIRCRAFT FROM ADSB ARE """,aircrafts, """"
    """)

    if len_air != len(aircrafts):

        len_air = len(read_aircraft())
            
        adsb_fig.add_trace(go.Scatter(x=[aircrafts[len_air-1][0]],
                            y=Y,
                            name = aircrafts[len_air-1][1],
                            showlegend=False,
                            visible = True,
    ...
    for i in range(0,len_air):
        adsb_fig.plotly_restyle({'x': [[aircrafts[i][0]]]}, i)
        adsb_fig.plotly_restyle({'name': aircrafts[i][1]}, i)
    

    for trace in adsb_fig.data:
        if(trace.x < (10,)):
...
def update_dsp_params(update_freq, en_pr, en_persist, persist_decay, max_bistatic_range, max_doppler, clutter_cancel_algo, dynrange_max, dynrange_min): #, input_value):
    """
    This is probably a leftover from kraken
    """
