    ...
    def run(self):
        ...
        while True:
            ...
            while self.run_processing:  
                if VERBOSE:
                    print("""
    ------Attempting to process a new frame------""")
...
#                        signal_window = np.ones(len(self.spectrum[1,:])) * -100
 #                       signal_window[max(self.max_index - self.fft_signal_width//2, 0) : min(self.max_index + self.fft_signal_width//2, len(self.spectrum[1,:]))] = max(self.spectrum[1,:])
                    ...
                    if self.en_PR and self.data_ready and self.channel_number > 1:
                        ...
                        if VERBOSE:
                            print("Wienertime: " + str((end-start) * 1000)) #this will print time in terminal, horrible to shut down kraken...
                        ...
                        end2 = time.time()
                        if VERBOSE:
                            print('bistatdopp time: ',1000.0*(end2-end))

                        que_data_packet.append(['RD_matrix', RD_matrix])
                        if VERBOSE:
                            print('SIZE OF RDMATRIX IS ',np.shape(RD_matrix))
                ...
                if VERBOSE:
                    print('latency time ',(1000*start_time-self.module_receiver.iq_header.time_stamp)*-1,' ms')
